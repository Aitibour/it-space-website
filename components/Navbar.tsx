...
export function Navbar() {
  ...
  function scheduleClose() {
    closeTimer.current = setTimeout(() => setDesktopOpen(false), 150);
  }
    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-md' : 'bg-gradient-to-b from-black/50 to-transparent'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7">
          ...
          {/* Services with JS-state dropdown — works across Row 2 strip */}
          <div
            className="relative"
          ...
          >
            ...
            >
              {t('services')}
              <ChevronDown size={13} className={`transition-transform duration-200 ${desktopOpen ? 'rotate-180' : ''}`} />
              {isServicesActive && <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-[#00B4FF] rounded-full" />}
            </Link>

            {/* Dropdown — JS state, stays open while mouse travels through Row 2 */}
            {desktopOpen && (
            <div
              className="absolute top-[calc(100%+48px)] left-1/2 -translate-x-1/2 w-[640px] bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50"
              onMouseEnter={openDropdown}
              onMouseLeave={scheduleClose}
            >
